...
class RoomsService(BaseService, ExceptionsHandler):
    async def get_rooms(self, hotel_id: int, dates: DateDep):
    ...
    async def get_room(self, room_id: int, hotel_id: int):
        room: FullRoomData = await self.get_room_and_check_existence(
            db=self.db, hotel_id=hotel_id, room_id=room_id, room_with_rel=True
        )
        return room

    async def add_room(self, room_data: RoomRequest, hotel_id: int):
        await self.get_hotel_and_check_existence(db=self.db, hotel_id=hotel_id)

        additionals_ids = await self.db.additionals.get_all_filtered_by_ids(
            ids_list=room_data.additionals_ids
        )  # type: ignore
        ...
        additionals = [
            RoomsAdditionalsRequest(additional_id=addit_id, room_id=room.id)
        ...
        added_room_with_rels = await self.get_room_and_check_existence(
            db=self.db, hotel_id=hotel_id, room_id=room.id, room_with_rel=True
        )
        return added_room_with_rels

    async def delete_room(self, room_id: int, hotel_id: int):
        await self.get_room_and_check_existence(
            db=self.db, hotel_id=hotel_id, room_id=room_id, room_with_rel=False
        )
    ...
    async def edit_room(
        ...
        await self.db.rooms.edit(
            room_data, id=room_id, hotel_id=hotel_id, exclude_fields={"additionals_ids"}
        )
        ...
        if "additionals_ids" in _room_data:
            ...
            await self.db.rooms_additionals.update_all(
                room_id=room_id, additionals_ids=room_data.additionals_ids
            )
        await self.db.commit()
