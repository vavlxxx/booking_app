"""added some extra fields and constraints

Revision ID: 4b15caf4e7cf
Revises: 0514c8fa3815
Create Date: 2025-06-14 12:45:31.267173
...
def upgrade() -> None:
    ...
    op.add_column("users", sa.Column("gender", sa.String(length=7), nullable=False))

    op.create_check_constraint(
        "check_birthday_validity",
        "users", 
        "birthday <= CURRENT_DATE"
    )
    op.create_check_constraint(
        "check_gender_validity",
        "users",
        "gender IN ('M', 'F')"
    )
