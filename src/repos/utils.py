...
def rooms_data_to_booking(date_from: date, date_to: date, hotel_id: int | None = None):
    rooms_count = (
    ...
    rooms_ids_by_hotel = select(RoomsOrm.id).select_from(RoomsOrm)

    if hotel_id is not None:
        rooms_ids_by_hotel = rooms_ids_by_hotel.filter_by(hotel_id=hotel_id)

    rooms_ids_by_hotel = rooms_ids_by_hotel.subquery(name="rooms_ids_by_hotel")

    empty_rooms_data_to_get = (
        select(rooms_left)
        .select_from(rooms_left)
        .filter(rooms_left.c.empty > 0, (rooms_left.c.id).in_(select(rooms_ids_by_hotel)))
    )
