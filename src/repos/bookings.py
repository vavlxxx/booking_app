...
class BookingsRepository(BaseRepository):
    ...
    async def get_today_checkin(self):
        query = (
            select(self.model).filter(self.model.date_from == date.today())  # type: ignore
        )
    ...
    async def add_booking(self, booking_data: BookingAdd, hotel_id: int):
        rooms_data = rooms_data_to_booking(booking_data.date_from, booking_data.date_to, hotel_id)
