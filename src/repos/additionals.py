...
class RoomsAdditionalsRepository(BaseRepository):
    ...
    async def update_all(self, room_id: int, additionals_ids: list[int]):
        ...
        delete_stmt = delete(self.model).filter(  # type: ignore
        ...
        add_new_rooms_addits = insert(self.model).from_select(  # type: ignore
            ["room_id", "additional_id"],
            select(room_id, AdditionalsOrm.id).filter(  # type: ignore
