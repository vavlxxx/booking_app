...
from src.utils.exceptions import ObjectNotFoundException, InvalidDataException
...
class RoomsRepository(BaseRepository):
    ...
    async def get_all_filtered_by_time(self, hotel_id, date_from, date_to):
        ...
        query = (
            ...
            .filter(
                self.model.id.in_(  # type: ignore
                    select(empty_rooms_data_to_get.c.id).select_from(empty_rooms_data_to_get)  # type: ignore
                )
            )
        )
        result = await self.session.execute(query)
        return [
            RoomsRelsMapper.map_to_domain_entity(obj) for obj in result.unique().scalars().all()
        ]

    async def get_one_or_none_with_rel(self, **filter_by) -> RoomsWithRels | None:
        query = (
            select(self.model).filter_by(**filter_by).options(joinedload(self.model.additionals))  # type: ignore
        )
