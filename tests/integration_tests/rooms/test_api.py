...
async def test_change_additionals_in_room(
    ...
    new_additionals_ids = get_random_ids(
        len(ADDITIONALS_EXAMPLES), exclude=additionals_ids_before_change
    )
    response = await ac.patch(
        url=f"hotels/{hotel_id}/rooms/{room_id}", json={"additionals_ids": new_additionals_ids}
    )
    assert response.status_code == 200
